<template>
  <div class="container">
    <h1 class="text-center mt-5" v-if="!restockOrder">
      ไม่มี restock order อยู่ในระบบ
    </h1>
    <div class="row justify-content-end mt-4">
      <div class="dropdown col-3 text-center">
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
          <a class="dropdown-item btn" @click="fetchRestockOrder('all')">All</a>
          <a class="dropdown-item btn" @click="fetchRestockOrder('pending')"
            >Pending</a
          >
            <a class="dropdown-item btn" @click="fetchRestockOrder('finished')"
            >Finished</a
          >
        </ul>
      </div>
    ...
    <table
      v-if="restockOrder"
      class="table container mt-4 table-secondary text-center"
    >
...
export default {
  ...
  async created() {
    await this.fetchRestockOrder("all");
  },
  methods: {
    async fetchRestockOrder(status) {
    ...
    infoHandler(order_id) {
      ...
      this.$swal({
        ...
        text: `สถานะ : ${orderShow.status}\n
                ...
                ราคาของสินค้าต่อชิ้น : ${orderShow.items[0].pivot.buyPrice.toFixed(
                  2
                )} บาท\n
                ราคาของสินค้าทั้งหมด : ${orderShow.items[0].pivot.total_item_price.toFixed(
                  2
                )} บาท\n
                ค่าขนส่ง : ${orderShow.ship_price.toFixed(2)} บาท\n
                ราคารวมทั้งหมด : ${orderShow.total_restock_price.toFixed(
                  2
                )} บาท\n
                วันที่สร้างคำสั่งซื้อ : ${this.dateFormat(
                  orderShow.created_at
                )}`,
      });
    },
    ...
    async statusHandler(order_id) {
      this.$swal({
        ...
        if (result) {
          ...
          res.status == "success"
            ? this.updateSccessHandler()
            : this.updateFailHandler();
        }
      });
    },
