<template>
    <div class="container">
        <div class="mt-5">
            <div class="card mb-3 mx-5" v-for="(item, index) in order.items" v-bind:key="index">
                <div class="row g-0">
                    <div class="col-md-3">
                        <img
        ...
        <div class="d-flex justify-content-between align-items-center mt-5">
            <h3 class="d-flex align-items-center">
                ...
                <button class="btn btn-danger fs-4 ms-3" @click="deleteOrder()">
                    ยกเลิก
                </button>
            </h3>
...
export default {
    ...
    methods: {
        ...
        async deleteOrder() {
            this.$swal({
                title: 'คุณแน่ใจที่จะลบคำสั้งซื้อนี้ใช่ไหม',
                icon: 'warning',
                buttons: true,
                dangerMode: true,
            }).then(async result => {
                if (result) {
                    ...
                    if (res.data.status === 'success') {
                        this.$swal('ลบคำสั้งซื้อสำเร็จ', '', 'success').then(() => {
                            this.$router.push('/orderlist');
                        });
                    } else if (res.data.status == 'fail') {
                        this.$swal('เกิดข้อผิดพลาดขึ้น', res.data.message, 'error');
                    } else {
                        this.$swal('เกิดข้อผิดพลาดขึ้น', 'กรุณารองอีกครั้ง', 'error');
                    }
                }
            });
        },
